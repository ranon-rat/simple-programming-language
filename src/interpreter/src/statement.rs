...
impl Interpreter {
    ...
    pub fn eval_for_loop(&mut self, for_loop: &ForLoop) -> (Types, ReasonsForStopping) {
        let ctx: Ctx = self;
        let mut interpreter = self.new_context(ctx);
        self.eval_expression(&for_loop.init, false);
        let condition = self.eval_expression(&for_loop.condition, for_loop.is_bool);
        while condition.to_number() > 0.0 {
            ...
            self.eval_expression(&for_loop.condition, for_loop.is_bool);
            self.eval_expression(&for_loop.increment, false);
        }
