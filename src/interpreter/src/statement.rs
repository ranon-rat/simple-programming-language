...
impl Interpreter {
    ...
    pub fn eval_ctx(&self, ctx: Ctx, lines: &Vec<Stmt>) -> (Types, ReasonsForStopping) {
        unsafe {
            match ctx.as_mut() {
                Some(interpreter) => {
                    return interpreter.eval_statement(ctx, lines);
                }
                None => return (Types::Number(0.0), ReasonsForStopping::Finished),
            }
        }
    }
    pub fn eval_block(&mut self, ctx: Ctx, lines: &Vec<Stmt>) -> (Types, ReasonsForStopping) {
        let new_ctx = self.new_context(ctx);
        return self.eval_ctx(new_ctx, lines);
    }
    ...
    pub fn eval_if_statement(&mut self, ctx: Ctx, if_stmt: &If) -> (Types, ReasonsForStopping) {
        ...
        if condition.to_number() > 0.0 {
            let new_ctx = self.new_context(ctx);
            return self.eval_ctx(new_ctx, &if_stmt.if_then);
        }
        for elif in &if_stmt.elif_then {
            let condition = self.eval_expression(ctx, &elif.condition, elif.condition_bool);
            if condition.to_number() > 0.0 {
                let new_ctx = self.new_context(ctx);
                return self.eval_ctx(new_ctx, &elif.then);
            }
        }
        let new_ctx = self.new_context(ctx);
        return self.eval_ctx(new_ctx, &if_stmt.else_then);
    }
    pub fn eval_while_loop(
        ...
        unsafe {
            match new_ctx.as_mut() {
                Some(interpreter) => {
                    let mut condition =
                        self.eval_expression(ctx, &while_loop.condition, while_loop.is_bool);
